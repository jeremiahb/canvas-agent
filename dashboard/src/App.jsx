...
function DocumentsPanel({ api }) {
  ...
  const savePaste = async () => {
    ...
    try {
      const res = await api.post("/documents/upload", {
        title: pasteTitle, text: pasteText, course_name: pasteCourse,
      });
  ...
  const saveFile = async () => {
    if (!uploadFile || !uploadCourse.trim()) {
      setMsg("Select a file and enter the course name."); return;
    }
    setSaving(true); setMsg("");
    try {
      const res = await api.uploadWithFields("/documents/upload-file", uploadFile, {
        course_name: uploadCourse,
  ...
  return (
    <div style={{ display: "flex", flexDirection: "column", gap: 24 }}>
      <div style={{ display: "flex", gap: 4, marginBottom: 4 }}>
        {["flagged", "paste", "file"].map((t) => (
          <button key={t} onClick={() => { setDocTab(t); setMsg(""); }} style={{
            padding: "6px 16px", borderRadius: 20, border: "none", cursor: "pointer",
            background: docTab === t ? "#1d4ed8" : "#1f2937", color: "#f9fafb",
            fontSize: 13, fontFamily: "inherit", fontWeight: docTab === t ? 600 : 400,
          }}>
            {t === "flagged" ? `Flagged (${flagged.length})` : t === "paste" ? "Paste Text" : "Upload File"}
          </button>
        ))}
      </div>
      ...
      {docTab === "flagged" && (
        <div>
          <p style={{ fontSize: 13, color: "#6b7280", marginBottom: 16 }}>
            These links were found on Canvas but could not be accessed automatically.
            Add them manually using Paste Text or Upload File.
          </p>
      ...
      {docTab === "paste" && (
        <Section title="Paste Reading Text">
          ...
          <input value={pasteTitle} onChange={(e) => setPasteTitle(e.target.value)}
            placeholder="Document title (e.g. Chapter 3 — Supply and Demand)"
            style={inputStyle} />
          <input value={pasteCourse} onChange={(e) => setPasteCourse(e.target.value)}
            placeholder="Course name (e.g. ECON 101)"
            style={{ ...inputStyle, marginTop: 8 }} />
          <textarea value={pasteText} onChange={(e) => setPasteText(e.target.value)}
            placeholder="Paste the reading content here…"
            rows={10}
      ...
      {docTab === "file" && (
        <Section title="Upload PDF or Word File">
          <p style={{ fontSize: 13, color: "#6b7280", marginBottom: 14 }}>
            Upload a PDF or .docx file directly. Max 20 MB.
          </p>
          ...
          <input value={uploadTitle} onChange={(e) => setUploadTitle(e.target.value)}
            ...
            style={inputStyle} />
          <input value={uploadCourse} onChange={(e) => setUploadCourse(e.target.value)}
